...
internal sealed class InputVectors : MapDataFrame<string, string>
{
        ...
        string? stateName)
    {
        ...
        foreach (RawRecvData recv in vecData)
        {
            if (indexName == null)
            {
        ...
        SummarizeResidues(vecData, out Dictionary<int, ResidueInfo>? residueInfoBySeq, out Dictionary<string, ResidueInfo>? residueInfoByIndex);

        Dictionary<string, ResidueInfo> residueInfoByRowKey = residueInfoByIndex
        ...
        Dictionary<string, double[]> summaryFields = vecData.ToDictionary(o => o.VectorName, o => o.SummaryFields);

        // Create the instance
        InputVectors obj = new(
    ...
    {
        Dictionary<string, HashSet<string>> chainDictByIndex = [];
        Dictionary<string, HashSet<AminoAcid>> resNameDictByIndex = [];
        Dictionary<string, HashSet<int>> resSeqDictByIndex = [];

        Dictionary<int, HashSet<string>> chainDictBySeq = [];
        Dictionary<int, HashSet<AminoAcid>> resNameDictBySeq = [];

        foreach (RawRecvData vec in vecData)
        {
            foreach (ResidueScore res in vec.ResidueScores)
            {
                ...
                {
                    // Chain IDs
                    if (chainDictByIndex.TryGetValue(key, out HashSet<string>? hsChains))
                        hsChains.Add(res.ChainId);
                    else
                        chainDictByIndex[key] = [res.ChainId];

                    // Residue names
                    if (resNameDictByIndex.TryGetValue(key, out HashSet<AminoAcid>? hsResNames))
                        hsResNames.Add(res.Residue);
                    else
                        resNameDictByIndex[key] = [res.Residue];

                    // Residue sequences
                    if (resSeqDictByIndex.TryGetValue(key, out HashSet<int>? hsResSeqs))
                        hsResSeqs.Add(res.ResidueSeq);
                    else
                        resSeqDictByIndex[key] = [res.ResidueSeq];
                }
                else
                {
                    ...
                    // Chain IDs
                    if (chainDictBySeq.TryGetValue(key2, out HashSet<string>? hsChains))
                        hsChains.Add(res.ChainId);
                    else
                        chainDictBySeq[key2] = [res.ChainId];

                    // Residue names
                    if (resNameDictBySeq.TryGetValue(key2, out HashSet<AminoAcid>? hsResNames))
                        hsResNames.Add(res.Residue);
                    else
                        resNameDictBySeq[key2] = [res.Residue];
                }
            }
        }
        ...
        (
            ...
            resSeqDictByIndex[idx].Count == 1 ? resSeqDictByIndex[idx].First() : null,
            idx,
            idx
        ));
